...
resource "aws_instance" "myEc2Public" {
  ...
  user_data              = <<-EOF
              ...
              unzip awscliv2.zip
              sudo ./aws/install
              aws ecr-public get-login-password --region us-east-1 | docker login --username AWS --password-stdin public.ecr.aws/d5z3t2m0
              sudo docker pull public.ecr.aws/d5z3t2m0/chaitanya:latest
              sudo docker run -d -p 8080:80 public.ecr.aws/d5z3t2m0/chaitanya:latest              
              EOF
